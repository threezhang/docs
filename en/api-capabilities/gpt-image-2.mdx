...
description: "Developer guide for GPT-Image-2, covering the default-group reverse routes, gpt-image-2-vip size support, and the Sora2Official official-transit route."
icon: "image"
---
...
| Default group | `gpt-image-2-vip` | Reverse Codex | **$0.03/call** | Supported | Not supported | Chat Completions, Images generation, Images edits |
...
| Reverse route with explicit image size | Default-group `gpt-image-2-vip` |
...
Default-group `gpt-image-2-vip` is the reverse Codex route. Use it when you need explicit output size on a reverse route.
...
<AccordionGroup>
  ...
  <Accordion title="Can gpt-image-2-vip accept quality?">
    No. `gpt-image-2-vip` supports `size` only. Use `gpt-image-2` in the `Sora2Official` group when you need `quality`.
  </Accordion>
