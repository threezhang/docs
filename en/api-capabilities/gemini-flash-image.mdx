...
|-------|-----------|----------|---------|-----------|------------|-------|----------------------|
| **Nano Banana 2** | 🆕 Latest | `gemini-3-pro-image-preview` | Preview | Gemini 3 | 1K/2K/4K | $0.05/image | Best quality, high resolution needed |
...
#### 1️⃣ Text-to-Image (Nano Banana 2, 4K)
...
# --- Parameters ---
PROMPT = "A futuristic city skyline at sunset, high detailed, 4k"
ASPECT_RATIO = "16:9"
IMAGE_SIZE = "4K"  # Only Nano Banana 2 supports 1K, 2K, 4K
...
def generate_text_to_image(api_key, prompt, aspect_ratio, image_size, output_dir, api_url):
    ...
    print(f"🚀 Starting text-to-image (Nano Banana 2, {image_size})...")
    print(f"📝 Prompt: {prompt}")
    print(f"📐 Aspect Ratio: {aspect_ratio}, Resolution: {image_size}")
    ...
    try:
        ...
        output_path = os.path.join(output_dir, f"text_to_image_4k_{timestamp}.png")
...
#### 2️⃣ Single Image-to-Image (Nano Banana)
...
# --- Parameters ---
INPUT_IMAGE_PATH = "test_cat.jpg"  # Ensure this image exists in current directory
PROMPT = "Turn this cat into a Van Gogh Starry Night style oil painting, thick brushstrokes, vibrant colors"
ASPECT_RATIO = "1:1"
OUTPUT_DIR = "."
...
def generate_image_to_image(api_key, input_image_path, prompt, aspect_ratio, output_dir, api_url):
    if not os.path.exists(input_image_path):
        print(f"❌ Error: Input image '{input_image_path}' not found. Please ensure the image is in the current directory.")
        return False
    ...
    print(f"🚀 Starting single image-to-image (Nano Banana)...")
    print(f"📝 Prompt: {prompt}")
    ...
    try:
        response = requests.post(api_url, headers=headers, json=payload, timeout=180)
            ...
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = os.path.join(output_dir, f"image_to_image_{timestamp}.png")
            with open(output_path, "wb") as f:
                f.write(raw_data)
...
if __name__ == "__main__":
    generate_image_to_image(API_KEY, INPUT_IMAGE_PATH, PROMPT, ASPECT_RATIO, OUTPUT_DIR, API_URL)
```

#### 3️⃣ Multi-Image Mixing (Nano Banana 2)
...
# --- Parameters ---
INPUT_IMAGE_PATHS = ["test_cat.jpg", "test_apple.jpg"]  # Ensure images exist
PROMPT = "Combine these images: A realistic scene of this cat eating this red apple on a wooden table"
ASPECT_RATIO = "16:9"
...
def generate_multi_image(api_key, input_image_paths, prompt, aspect_ratio, image_size, output_dir, api_url):
    ...
    print(f"🚀 Starting multi-image mixing (Nano Banana 2, {image_size})...")
    print(f"📝 Prompt: {prompt}")
    print(f"🖼️ Input images: {', '.join(input_image_paths)}")
    ...
    try:
        ...
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = os.path.join(output_dir, f"multi_image_mix_{timestamp}.png")
        with open(output_path, "wb") as f:
            f.write(raw_data)
