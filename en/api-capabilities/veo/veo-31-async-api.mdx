...
| `input_reference` | file | ❌ | Local image file (required for Image-to-Video models, multipart/form-data) |
...
def create_video_task(prompt, model="veo-3.1", image_path=None):
    """Create video generation task"""
    url = f"{BASE_URL}/videos"
    headers = {"Authorization": f"Bearer {API_KEY}"}
    
    if image_path:
        # Image-to-Video: Use multipart/form-data to upload image
...
if __name__ == "__main__":
    ...
    # Image-to-Video Example (Make sure test.jpg exists in current directory)
    # create_video_task(
