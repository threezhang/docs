...
<Card>
  **Image generation route update**
  ...
  - 🚀 **New routes**:
    ...
    - `gpt-image-2-vip` reverse Codex route is now available with `size` support for workflows that need explicit output dimensions
  ...
  <Info>
    Default-group reverse routes support Chat Completions, Images Generations, and Images Edits. Use `gpt-image-2` for standard generation, and use `gpt-image-2-vip` when you need the `size` parameter.
  </Info>
</Card>
...
- April 2026: GPT-Image-2 reverse routes launched with the new `gpt-image-2-vip` size-parameter route; `sora_image` official route went offline; Claude Opus 4.7 / Thinking launched; Nano Banana series pricing updated
