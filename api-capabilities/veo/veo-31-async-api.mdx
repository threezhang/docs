...
```bash 步骤1: 创建任务（图生视频 - 单图）
# 注意：需要使用 multipart/form-data 格式上传图片
curl -X POST "https://api.laozhang.ai/v1/videos" \
  -H "Authorization: Bearer YOUR_API_KEY" \
...
```bash 步骤1: 创建任务（图生视频 - 首尾帧）
# 使用两张图片作为首帧和尾帧，生成平滑过渡的视频
curl -X POST "https://api.laozhang.ai/v1/videos" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: multipart/form-data" \
  -F "model=veo-3.1-landscape-fl" \
  -F "prompt=让这个画面动起来，加点未来科技感" \
...
| `input_reference` | file | ❌ | 图片文件，支持 1-2 张（2张时为首尾帧模式，multipart/form-data 格式）|
