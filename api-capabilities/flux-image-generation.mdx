---
title: Flux 图像生成 API
description: "FLUX AI 绘图 API：业界领先图像质量，支持 3:7 到 7:3 多尺寸。$0.035/张，官方 65 折。OpenAI 兼容格式，国内直连。"
icon: "sparkles"
---
...
Flux 是业界领先的图像生成模型，通过 老张API 的 OpenAI 兼容接口，您可以轻松调用 Flux Kontext Pro 和 Max 模型生成高质量图像。我们提供了比官方更优惠的价格，结合汇率优势和充值加赠，为您节省更多成本。
...
| 模型 | 模型 ID | 老张API 价格 | 官方价格 | 节省比例 | 特点 |
|------|---------|-----------|----------|----------|------|
...
<Tip>
💡 **价格优势**：通过 老张API 平台调用 Flux 模型，结合汇率优势（1:7）和充值加赠政策（单次大额充值加赠 10%），比官方定价节省超过 25%！
</Tip>
...
def calculate_flux_costs(num_images, model_type="pro"):
    ...
    print(f"老张API价格: ${laozhang_cost:.2f}")
...
- [价格对比计算器](https://api.laozhang.ai/account/pricing) - 实时价格查询
- [在线体验 Demo](https://imagen.laozhang.ai) - 测试 Flux 效果
