...
description: "Nano Banana Edit (Standard): 基于 Gemini 2.5 Flash 的高性价比图像编辑。$0.025/次，支持多图合成。"
icon: "paintbrush"
---

<Warning>
**⚠️ 模型即将下线通知**  
`gemini-2.5-flash-image-preview` 预览版即将下线。

**推荐迁移到正式版**:
- ✅ 新版本：[Gemini 2.5 Flash Image 编辑](/api-capabilities/gemini-flash-image-edit)（正式版，更稳定）
- 🎁 新功能：支持自定义纵横比（16:9、9:16 等 10 种比例）
- 💰 价格不变：仍为 $0.025/次
- 🔄 迁移简单：只需更新模型名称即可

[查看新版文档 →](/api-capabilities/gemini-flash-image-edit)
</Warning>

## 模型简介

Nano Banana (Standard) 图像编辑功能基于 Google `gemini-2.5-flash-image-preview` 模型，通过对话补全接口实现对现有图片的智能编辑和改造。支持单张或多张图片的输入，可以实现图像合成、元素添加、风格转换等高级编辑功能。
