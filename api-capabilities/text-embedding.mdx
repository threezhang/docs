...
description: "Text Embedding API：将文本转为向量表示，用于语义搜索、RAG、相似度计算。支持 OpenAI、Cohere、BGE 等多种模型。"
icon: "binary"
---
...
| `text-embedding-3-large` | 3072 | 效果最好，维度高 | 高精度需求 |
| `text-embedding-ada-002` | 1536 | 经典模型，兼容性好 | 已有系统迁移 |
...
<Tip>
  老张API 提供更优惠的价格，详见 [定价页面](/pricing)。
</Tip>

## 常见问题

<AccordionGroup>
  <Accordion title="向量维度多少合适？">
    - 通用场景：1536 维（默认）足够
    - 存储受限：可降到 512 或 256 维
    - 高精度需求：使用 3072 维的 large 模型
  </Accordion>

  <Accordion title="单次最多处理多少文本？">
    建议单次请求不超过 2000 个文本，过多可能超时。
  </Accordion>

  <Accordion title="文本长度限制是多少？">
...
<CardGroup cols={2}>
  ...
  <Card title="LangChain 集成" icon="link" href="/scenarios/engineering/langchain">
    在 LangChain 中使用 Embedding
  </Card>
  <Card title="模型信息" icon="database" href="/api-capabilities/model-info">
    查看所有支持的模型
  </Card>
  <Card title="Dify 配置" icon="puzzle" href="/scenarios/engineering/dify">
    在 Dify 中配置向量模型
  </Card>
</CardGroup>
