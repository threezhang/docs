...
- 💰 **价格**：Nano Banana $0.025/张，Nano Banana 2 $0.05/张
</Note>
...
- ⚡ 平均 10 秒生成高质量图片
- 💰 超值价格：Nano Banana 2 $0.05/张，Nano Banana $0.025/张
</Note>
...
|------|---------|----------|-----------|----------|----------|--------|------|
| **Nano Banana 2** | `gemini-3-pro-image-preview` | 按次计费 | $0.05/张 | $0.04/张 | - | 1K/2K/4K | ~10秒 |
...
<Tip>
💰 **价格优势详解**  
- **Nano Banana 2**：$0.05/张（老张API）vs $0.04/张（官网）
- **Nano Banana**：$0.025/张（比官网便宜 37.5%）
...
<Accordion title="展开查看完整 Python 脚本（文生图 + 4K 支持）">
...
# ASPECT_RATIO = "16:9"
# IMAGE_SIZE = None  # 稳定版不支持此参数
# ============================

def generate_image(api_url, api_key, prompt, aspect_ratio="1:1", image_size=None):
    """生成图片并保存"""
    ...
    try:
        ...
        filename = f"gemini{size_tag}_{timestamp}.png"
...
<Accordion title="展开查看单图生图脚本（图生图 + 风格迁移）">
...
def image_to_image(api_url, api_key, input_image, prompt, aspect_ratio="1:1", image_size="2K"):
    """图生图"""
    ...
    # 编码图片
    mime_type, image_data = encode_image(input_image)
    ...
    try:
        ...
        result = response.json()
        image_data_out = result["candidates"][0]["content"]["parts"][0]["inlineData"]["data"]
        
        # 保存结果
        output_file = f"output_styled_{os.path.basename(input_image)}"
        with open(output_file, "wb") as f:
            f.write(base64.b64decode(image_data_out))
...
<Accordion title="展开查看多图混合脚本（创意合成）">

```python
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Gemini 多图混合工具 - 最多支持 14 张图片
"""
...
# 准备多张图片（示例：猫和苹果）
INPUT_IMAGES = ["cat.jpg", "apple.jpg"]
PROMPT = "Create a photorealistic scene: this cute cat is curiously sniffing this red apple on a wooden table"
ASPECT_RATIO = "16:9"
IMAGE_SIZE = "2K"
# ============================

def multi_image_mix(api_url, api_key, image_paths, prompt, aspect_ratio="16:9", image_size="2K"):
    """多图混合生成"""
    
    print("="*60)
    print(f"🎨 Gemini 多图混合")
    print("="*60)
    print(f"🖼️  输入图片数量: {len(image_paths)}")
...
<AccordionGroup>
  <Accordion title="什么是 Nano Banana 和 Nano Banana 2？">
    ...
    **主要区别**：
    ...
    - 价格：Nano Banana 2 $0.05/张，Nano Banana $0.025/张
    - 调用方式完全相同
  </Accordion>
  ...
  <Accordion title="Nano Banana 2 和 Nano Banana 哪个更好？">
    ...
    **价格差异**：
    - Nano Banana 2：$0.05/张（支持高分辨率）
    - Nano Banana：$0.025/张（固定 1K）
  </Accordion>
...
<AccordionGroup>
  <Accordion title="2025-01-XX：4K 分辨率及高级特性上线">
    **🚀 Nano Banana 2 (Gemini 3 Pro Image) 重大更新**
    ...
    - **💰 价格更新**：Nano Banana 2 $0.05/张，Nano Banana $0.025/张
