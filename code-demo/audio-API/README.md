# Gemini 音频理解 API 示例

使用 Gemini 2.5 Pro 模型进行音频转录、分析和理解。

## 文件说明

| 文件 | 说明 | 用途 |
|------|------|------|
| `gemini_audio_demo.py` | 内部测试版本（已填入 API Key） | 开发测试 |
| `gemini_audio_example.py` | 用户示例版本（需要填入 API Key） | 分发给用户 |
| `test_audio.mp3` | 测试音频文件 | 测试用 |

## 功能特点

- ✅ 支持音频转录（转文字）
- ✅ 支持音频内容分析和总结
- ✅ 支持多种音频格式（MP3, WAV, M4A, WEBM等）
- ✅ 自动保存结果（TXT + JSON 格式）
- ✅ 高准确度的中英文识别
- ✅ 使用 requests 库，无需额外 SDK

## 快速开始

### 1. 安装依赖

```bash
pip install requests
```

**注意**：只需要安装 `requests` 库，不需要安装 `openai` 库！

### 2. 配置 API Key

编辑 `gemini_audio_example.py`，找到顶部的 API_KEY 变量：

```python
# ============================================================
# ⚠️ 请在这里填入您的 API Key
# 获取地址: https://api.laozhang.ai/
# ============================================================
API_KEY = "sk-your-api-key-here"  # ← 替换为您的实际 API Key
# ============================================================
```

### 3. 运行测试

```bash
# 使用默认测试音频
python gemini_audio_example.py

# 或指定自定义音频文件
python gemini_audio_example.py /path/to/your/audio.mp3
```

## 使用示例

### 示例 1：音频转录

```python
from gemini_audio_example import gemini_audio_analysis

result = gemini_audio_analysis(
    question="请转录这段音频的内容",
    audio_path="test_audio.mp3"
)

print(result['result'])
```

### 示例 2：音频内容总结

```python
result = gemini_audio_analysis(
    question="请总结这段音频的主要内容",
    audio_path="your_audio.mp3"
)
```

### 示例 3：音频分析

```python
result = gemini_audio_analysis(
    question="这段音频讨论了什么主题？情绪如何？",
    audio_path="your_audio.mp3",
    model="gemini-2.5-pro"
)
```

### 示例 4：自定义 API Key

```python
result = gemini_audio_analysis(
    question="请转录这段音频",
    audio_path="test.mp3",
    api_key="sk-your-custom-key"
)
```

## 支持的模型

| 模型 | 特点 | 推荐场景 |
|------|------|---------|
| `gemini-2.5-pro` | 高准确度，强大的理解能力 | 专业转录、深度分析 |
| `gemini-2.0-flash-exp` | 速度快，成本低 | 实时处理、批量任务 |

## 支持的音频格式

- MP3 (.mp3)
- WAV (.wav)
- M4A (.m4a)
- MP4 音频 (.mp4)
- MPEG (.mpeg, .mpga)
- WEBM (.webm)

**文件大小限制**: 建议不超过 20MB

## API 参数说明

### gemini_audio_analysis()

```python
def gemini_audio_analysis(question, audio_path, model="gemini-2.5-pro", api_key=None)
```

**参数:**
- `question` (str, 必需): 要问的问题
- `audio_path` (str, 必需): 本地音频文件路径
- `model` (str, 可选): 模型名称，默认 "gemini-2.5-pro"
- `api_key` (str, 可选): API 密钥，不提供则使用全局 API_KEY 变量

**返回:** dict
```python
{
    "result": "音频转录或分析结果",
    "model": "gemini-2.5-pro",
    "audio_path": "/path/to/audio.mp3",
    "question": "请转录这段音频的内容",
    "success": True  # 或 False
}
```

## 输出文件

运行后会自动生成两个文件：

1. **TXT 格式**: `gemini_audio_analysis_YYYYMMDD_HHMMSS.txt`
   - 包含完整的元数据和结果
   - 易于阅读和分享

2. **JSON 格式**: `gemini_audio_analysis_YYYYMMDD_HHMMSS.json`
   - 结构化数据
   - 便于程序处理

## 提示词建议

根据不同需求调整提问内容：

| 需求 | 推荐提示词 |
|------|-----------|
| 转录 | "请转录这段音频的内容" |
| 总结 | "请用3-5句话总结这段音频的主要内容" |
| 翻译 | "请将这段音频转录并翻译成英文" |
| 分析 | "请分析这段音频的情绪基调和讨论主题" |
| 提取 | "请提取音频中提到的关键数字和日期" |

## 常见问题

### Q: 为什么使用 requests 而不是 openai 库？

A: 使用 requests 库更加轻量级，不需要安装额外的 SDK，代码更简单易懂，适合学习和定制。

### Q: 如何提高识别准确度？

A: 
1. 使用 `gemini-2.5-pro` 模型（准确度更高）
2. 提供清晰、无噪音的音频
3. 使用具体明确的提示词
4. 如果是专业领域，可以在提示词中提供上下文

### Q: 支持哪些语言？

A: Gemini 支持多种语言，包括：
- 中文（简体/繁体）
- 英文
- 日文
- 韩文
- 以及其他主流语言

### Q: 音频太大怎么办？

A:
1. 使用音频压缩工具降低比特率
2. 将长音频分割为多个片段
3. 转换为 MP3 格式（通常更小）

### Q: 返回 API 错误怎么办？

A:
1. 检查 API Key 是否正确
2. 确认账户余额充足
3. 检查网络连接
4. 查看错误信息中的详细提示

## 代码结构

```
audio-API/
├── gemini_audio_demo.py     # 内部测试版本（已填入 Key）
├── gemini_audio_example.py  # 用户示例版本（需填入 Key）
├── test_audio.mp3           # 测试音频文件
├── README.md                # 本文档
└── 快速开始.md              # 快速入门指南
```

## 测试音频说明

目录中包含 `test_audio.mp3` 测试文件，内容为：
> "这是一段音频转录测试。人工智能技术正在快速发展，语音识别的准确率越来越高。"

您可以用自己的音频文件替换或添加更多测试文件。

## 技术说明

### API 调用方式

代码使用标准的 HTTP POST 请求调用 OpenAI 兼容的 Chat Completions API：

```python
response = requests.post(
    "https://api.laozhang.ai/v1/chat/completions",
    headers={
        "Content-Type": "application/json",
        "Authorization": f"Bearer {api_key}"
    },
    json=payload
)
```

### 音频处理

音频文件通过 base64 编码后嵌入到请求中：

```python
audio_b64 = base64.b64encode(audio_data).decode()
audio_url = f"data:audio/mp3;base64,{audio_b64}"
```

## API 费用

请参考 [老张 API 定价页面](https://docs.laozhang.ai/pricing) 了解详细费用信息。

## 相关资源

- 📖 [API 文档](https://docs.laozhang.ai/)
- 🔑 [获取 API Key](https://api.laozhang.ai/)
- 💬 技术支持：参考文档中的联系方式

## 许可证

本示例代码仅供学习和参考使用。
