...
你管理以下多个站点，研究报告必须考虑将关键词分配到最合适的站点，而非默认全部给 laozhang.ai：

| 站点 | 内容定位 | 主题边界 |
|------|---------|---------|
| **laozhang.ai** | AI API 中转主站 | 产品页/文档/定价；不做纯博客型 SEO 文章 |
| **blog.laozhang.ai** | AI API 技术博客 | 宽泛 AI API 内容：接入教程/模型对比/评测/开发者实践（不属于下面两站主题的都放这里） |
| **aifreeapi.com** | 免费/便宜 AI API 资源站 | 专注 free tier/限额/配额/省钱策略/cheap API；凡与"免费"或"低成本"强相关的词优先考虑此站 |
| **yingtu.ai** | AI 图像生成工具/资讯博客 | 专注图像生成/图像编辑/提示词/图像模型评测；凡与图像 AI 强相关的词优先考虑此站 |

**站点分配优先级规则**：
1. 主题匹配优先：图像相关词 → yingtu.ai；免费/cheap API 词 → aifreeapi.com；其余 AI API 技术词 → blog.laozhang.ai
2. 已有内容优先：哪个站已有相关页面，就在那个站优化，不要跨站重复建设
3. 避免自相残杀：同一关键词集群内，不同站点应覆盖不同角度，不要互相抢排名
4. laozhang.ai 主站只做产品/文档/定价页，不做普通博客文章

### 其他设置

- 目标：通过 SEO 和 GEO 占据搜索入口，筛选真正值得做的关键词与页面，并分配到最合适的站点
- 输出语言：中文
...
- **B5**：Agent + WebSearch（自有资产盘点，扫描全部4个站点：laozhang.ai、blog.laozhang.ai、aifreeapi.com、yingtu.ai）
...
2. **"{keyword} API 中转"** 或 **"{keyword} 国内使用"** — 与自有站点业务最相关的中文搜索
   - 同上操作（含降级策略）
...
#### Agent B5 — 自有资产盘点（全站扫描）

任务：检查所有自有站点（laozhang.ai、blog.laozhang.ai、aifreeapi.com、yingtu.ai）已有的与该关键词相关的内容，避免重复建设和跨站自相残杀。

具体操作：
1. 使用 WebSearch 依次搜索所有站点（共 8 次搜索）：
   - "site:laozhang.ai {keyword}"
   - "site:blog.laozhang.ai {keyword}"
   - "site:aifreeapi.com {keyword}"
   - "site:yingtu.ai {keyword}"
   - 再用核心词重复一轮（去掉版本号等修饰词）：
     - "site:laozhang.ai {keyword核心词}"
     - "site:blog.laozhang.ai {keyword核心词}"
     - "site:aifreeapi.com {keyword核心词}"
     - "site:yingtu.ai {keyword核心词}"
2. 对每个找到的已有页面，记录：
   - 所在站点
   - 页面 URL
   - 页面标题
...
3. 评估：
   - 哪个站点已有覆盖？覆盖程度如何？
   - 是否存在可以优化而非新建的页面？
   - 是否存在跨站重复内容（多个站点在抢同一个词）？
   - 已有页面可以为新文章提供哪些内链支撑（含跨站链接机会）？

输出格式：
- 已有页面列表（**站点** + URL + 标题 + 类型 + 当前状态）
- 跨站重复风险提示（如多个站点覆盖了同一关键词，需区分角度或合并）
- 优化建议 vs 新建建议（精确到具体站点）
- 内链机会列表（已有页面 → 新文章的链接机会，含跨站链接）
...
- 与自有站点的业务方向有明确承接关系（API 中转 / 免费 API / 图像生成 中至少一项）
- 内容可以做出差异化
- Google Trends 趋势不是下降（上升/稳定/刚爆发优先）
- 所有自有站点均未有优质页面充分覆盖该词（B5 盘点结果）
...
对每个优先做的关键词/角度，判断与自有站点业务网络的关系：
- **直接转化**：用户搜这个词 → 直接需要 API 中转服务
- **间接转化**：用户搜这个词 → 了解信息 → 发现可以用更便宜的方式调用 → 转化
- **品牌承接**：用户搜热点词 → 看到自有站点内容 → 建立品牌认知
- **不相关**：该词与所有自有站点业务均无直接关联

#### Step 2.6 — 站点分配判断

对每个"优先做"和"可做但不优先"的关键词，基于以下决策树判断应由哪个站点发布：

**分配决策树**（按顺序判断）：

1. **B5 已有覆盖**：某站点已有相关页面 → 优先在该站优化，不跨站新建
2. **主题匹配**：
   - 关键词与图像生成/图像编辑/图像模型/提示词强相关 → **yingtu.ai**
   - 关键词与 free/cheap/free tier/省钱/quota/限额强相关 → **aifreeapi.com**
   - 关键词是产品功能/定价/API 文档/中转介绍 → **laozhang.ai**（主站）
   - 其余 AI API 技术内容（接入教程/模型对比/评测/开发者实践）→ **blog.laozhang.ai**
3. **受众匹配**：哪个站点的读者群体最可能是这个关键词的搜索者？
4. **主题权重**：哪个站在相关主题上已有更多内容积累（= 更高主题权重，更容易排名）？
5. **跨站角度区分**：若两个站点都适合同一关键词 → 必须区分角度（如 yingtu.ai 做普通用户视角，blog.laozhang.ai 做开发者 API 视角），不得同一角度两站都写

**输出要求**：每个关键词必须明确标注"推荐站点：{站点域名}"，不得留空。
...
- **自有资产现状**：[各站点已有 X 个相关页面 / 无覆盖] — [优化已有 or 新建]
- **推荐主力发布站点**：[站点域名] — [一句话说明为何选此站]
- **对业务网络的核心价值**：[流量 / 转化 / 品牌认知 / GEO 引用]
- **最终判断**：[做 / 不做 / 换角度做]
...
每个关键词必须包含：理由（引用 Phase 1 数据）+ 推荐站点（来自 Step 2.6）。此层关键词数量必须不少于 10 个。如果不足 10 个，从 B1 短词根补全、A2 标题反向提词、B3 竞品覆盖词中补充。

以表格形式输出：

| 关键词 | 理由（引用数据） | 推荐站点 | 业务匹配 |
|--------|----------------|---------|---------|
| ... | ... | yingtu.ai / aifreeapi.com / blog.laozhang.ai | 直接转化/间接转化/品牌承接 |
...
**自有资产附录**：将 B5 发现的所有站点已有相关页面列出（站点 + URL + 标题 + 优化/新建建议），并标注跨站重复风险。
...
基于"优先做"层的 10+ 关键词，规划具体的文章列表。不要只给 1 篇文章建议，必须给出完整的内容矩阵。每篇文章必须明确指定发布站点（来自 Step 2.6 站点分配判断）。

#### 7.1 文章优先级列表

按优先级排序，列出所有建议撰写的文章（不少于 5 篇），每篇包含：

| 优先级 | 文章主题 | 目标关键词（可多个） | 页面类型 | 推荐 Title | 推荐 URL slug | **发布站点** | 页面角色 |
|--------|----------|---------------------|----------|-----------|--------------|------------|----------|
| P0 | ... | ... | ... | ... | ... | yingtu.ai / aifreeapi.com / blog.laozhang.ai | pillar/support/比较/教程 |
| P1 | ... | ... | ... | ... | ... | ... | ... |
| P2 | ... | ... | ... | ... | ... | ... | ... |
| ... | ... | ... | ... | ... | ... | ... | ... |
...
- **结论**：[做 / 不做 / 换角度做]
- **建议文章总数**：[X 篇]
- **P0 首发页**：[一句话说明] — 发布站点：[站点]
- **P1 第二批（1-3 篇）**：[列出，每篇注明发布站点]
- **P2 第三批（1-3 篇）**：[列出，每篇注明发布站点]
- **当前最不值得投入的方向**：[明确指出]
- **时效性提醒**：[列出报告中涉及的、需要定期更新的时效性信息，如模型版本、价格数据等]

### 十、站点协同策略

基于全部文章规划，说明4个站点在这个关键词集群上如何分工协作：

1. **各站点分工总览**：
   - yingtu.ai 负责：[哪些角度/文章]
   - aifreeapi.com 负责：[哪些角度/文章]
   - blog.laozhang.ai 负责：[哪些角度/文章]
   - laozhang.ai 负责：[哪些产品/文档页（如有）]

2. **跨站自相残杀风险提示**：
   - [列出存在角度重叠的文章对，以及如何区分避免互相竞争]
   - 如无风险，注明"无跨站重叠风险"

3. **跨站内链机会**：
   - [哪些文章应跨站互链，用什么锚文本]
   - 例：yingtu.ai 图像教程 → 链接到 aifreeapi.com 省钱方案（锚文本："更便宜的 API 调用"）

4. **站点优先级建议**：
   - 最先动哪个站？为什么？
   - 哪个站的内容对整体 SEO 集群贡献最大？
...
5. **业务视角**：站在自有站点业务网络（API 中转 + 免费 API + 图像生成）的角度，不是站在媒体/资讯站角度
...
15. **自有资产盘点**：报告必须包含 B5 的全站（4个站点）盘点结果，标注跨站重复风险，避免跨站自相残杀
16. **社区痛点**：报告必须包含 B4 的 Reddit/论坛用户问题数据，用于长尾词发现和内容差异化
17. **站点分配**：报告中每个"优先做"和"可做但不优先"的关键词必须标注推荐站点；每篇文章建议必须标注发布站点；必须包含第十章"站点协同策略"
